...
// embID is an arbitrary number for signing messages. This will make it
// clear that the signature comes from the Emb blockchain.
...
func stamp(value any) ([]byte, error) {
	...
	stamp := []byte(fmt.Sprintf("\x19Emb Signed Message:\n%d", len(v)))
...
func toSignatureValues(sig []byte) (v, r, s *big.Int) {
	r = big.NewInt(0).SetBytes(sig[:32])
	s = big.NewInt(0).SetBytes(sig[32:64])
	v = big.NewInt(0).SetBytes([]byte{sig[64] + embID})
