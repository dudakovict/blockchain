...
type Tx struct {
	...
	FromID  acc.AccountID `json:"from"`
	ToID    acc.AccountID `json:"to"`
...
func NewTx(chainID uint16, nonce uint64, fromID acc.AccountID, toID acc.AccountID, value uint64, tip uint64, data []byte) (Tx, error) {
